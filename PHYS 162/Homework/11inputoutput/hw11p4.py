...
print("Problem 4 \t Graphing the 'slopeData.dat file in Python\n")
...
plt.title('Graph of "slopeData.dat": $y = sqrt(x)$')
plt.xlabel(r'$x$')
plt.ylabel(r'$y$')

# Read in 'outputData.dat'.
outputDataX, outputDataY = np.loadtxt("outputData.dat", np.float64, usecols={0, 1}, unpack=True)
slopeDataX, slopeDataY = np.loadtxt("slopeData.dat", np.float64, usecols={0, 1}, unpack=True)

# plt.xlim(0, 10)
# plt.ylim(0, 1000)

plt.plot(outputDataX, outputDataY, label='$sqrt(x)$')
plt.plot(slopeDataX, slopeDataY, label='$slope$')

plt.legend()

# Show & export the graph.
plt.savefig("Problem 4 - Slope.pdf", dpi = 72)
plt.show()
